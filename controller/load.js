...
exports.addUserLoad = async function (req, res) {
  const { userId, role } = req.credential;
  const {error}= validate.newLoadValidate(req.body)
...
exports.postUserLoadById = async function (req, res) {
  ...
  try {
    ...
    const filterTrucks = trucks.filter(
      (truck) =>
        truck.width >= load.dimensions.width &&
        truck.length >= load.dimensions.length &&
        truck.height >= load.dimensions.height &&
        truck.weight >= load.payload
    );

    if (filterTrucks.length < 1) {
